...
@property (nonatomic, strong) UIBarButtonItem *pickerViewBarButtonItemRemove;
...
-(void)dismissCancel {
    [MMPickerView dismissWithCompletionCancel:self.onDismissCompletion];
}
+(void)dismissWithCompletionCancel:(void (^)(NSString *))completion {
    [[self sharedView] setPickerHiddenCancel:YES callBack:completion];
}
-(void)setPickerHiddenCancel: (BOOL)hidden
...
-(void)initializePickerViewInView: (UIView *)view
  ...
  _pickerContainerView = [[UIView alloc] initWithFrame:CGRectMake(0.0, _pickerViewContainerView.bounds.size.height - 260.0, SCREEN_SIZE.width, 260.0)];
  ...
  [_pickerViewBarButtonItem setTintColor:buttonTextColor];
    ...
    _pickerViewBarButtonItemRemove = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(dismissCancel)];
    [_pickerViewBarButtonItemRemove setTintColor:buttonTextColor];
  ...
  */
  ...
  //Add pickerView
  _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0.0, 44.0, SCREEN_SIZE.width, 216.0)];
